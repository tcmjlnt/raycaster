...
/*   Updated: 2025/09/25 20:33:30 by tjacquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_ray_line(t_mlx_data *data, int x0, int y0, int x1, int y1) // a expliciter
{
	...
	while (1)
	{
		if (x >= 0 && x <= data->window_width && y>= 0 && y<= data->window_height)
		{
			...
			*(unsigned int*)(data->ray_data + offset) = 0xFFFF00;		}
...
void	draw_ray(t_mlx_data *data, t_player_data *player) // a expliciter
{
...
void	render_map(t_mlx_data *data, t_player_data *player)
{
	...
	for (y = 0; y < 8; y++)
	{
		for (x = 0; x < 8; x++)
		{
			render_tile(data, char_to_tile(data->map.grid[y][x]), x, y);
		}
	}
...
int	render_loop(t_player_data *player)
{
	...
	clear_ray_image(player->mlx_data_pointer);
